 ...
 */
static inline SPIInterruptInfo_t *get_spi_interrupt_info(const SPI_TypeDef *spi_base_addr) {
  const volatile SPI_TypeDef *spi_addrs[SPIS_NUM] = SPIS;
  for (int i = 0; i < SPIS_NUM; i++) {
    if (spi_addrs[i] == spi_base_addr) return &spi_interrupt_info[i];
  }
  return NULL;
}
...
int spi_init(const SPIHandle_t *spi_handle) {
  ...
  if (cfg->interrupt_setup.en == SPI_INTERRUPT_ENABLE) {
    ...
    // For handling during the interrupt
    SPIInterruptInfo_t *info = get_spi_interrupt_info(spi_reg);
    info->type = cfg->interrupt_setup.type;
    info->eles_left = info->length = cfg->interrupt_setup.length;
    info->is_busy = 0;
  }
